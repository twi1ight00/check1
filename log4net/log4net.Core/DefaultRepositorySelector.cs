...
public class DefaultRepositorySelector : IRepositorySelector
{
	...
	public ILoggerRepository GetRepository(string repositoryName)
	{
		...
		lock (this)
		{
			if (!(m_name2repositoryMap[repositoryName] is ILoggerRepository result))
			{
	...
	public ILoggerRepository CreateRepository(Assembly repositoryAssembly, Type repositoryType, string repositoryName, bool readAssemblyAttributes)
	{
		...
		lock (this)
		{
			...
			{
				LogLog.Debug(declaringType, string.Concat("Creating repository for assembly [", repositoryAssembly, "]"));
				...
				LogLog.Debug(declaringType, string.Concat("Assembly [", repositoryAssembly, "] using repository [", repositoryName2, "] and repository type [", repositoryType2, "]"));
	...
	public ILoggerRepository CreateRepository(string repositoryName, Type repositoryType)
	{
		...
		lock (this)
		{
			...
			if (m_alias2repositoryMap[repositoryName] is ILoggerRepository loggerRepository2)
			{
	...
	private void GetInfoForAssembly(Assembly assembly, ref string repositoryName, ref Type repositoryType)
	{
		...
		try
		{
			...
			{
				LogLog.Debug(declaringType, string.Concat("Assembly [", assembly, "] does not have a RepositoryAttribute specified."));
				return;
			}
			if (customAttributes.Length > 1)
			{
				LogLog.Error(declaringType, string.Concat("Assembly [", assembly, "] has multiple log4net.Config.RepositoryAttribute assembly attributes. Only using first occurrence."));
			}
			if (!(customAttributes[0] is RepositoryAttribute repositoryAttribute))
			{
				LogLog.Error(declaringType, string.Concat("Assembly [", assembly, "] has a RepositoryAttribute but it does not!."));
				return;
			}
			...
			if (repositoryAttribute.RepositoryType != null)
			{
				...
				{
					LogLog.Error(declaringType, string.Concat("DefaultRepositorySelector: Repository Type [", repositoryAttribute.RepositoryType, "] must implement the ILoggerRepository interface."));
				}
			}
		}
		catch (Exception exception)
	...
	private void ConfigureRepository(Assembly assembly, ILoggerRepository repository)
	{
		...
		if (result)
		{
			...
			{
				LogLog.Error(declaringType, "DefaultRepositorySelector: Exception while parsing log4net.Config file physical path [" + text2 + "]", exception);
			}
			try
			{
			...
			{
				LogLog.Error(declaringType, "DefaultRepositorySelector: Exception calling XmlConfigurator.ConfigureAndWatch method with ConfigFilePath [" + text2 + "]", exception);
				return;
			}
		}
		Uri uri = null;
		...
		if (uri != null)
		{
			...
			{
				LogLog.Error(declaringType, string.Concat("Exception calling XmlConfigurator.Configure method with ConfigUri [", uri, "]"), exception);
			}
		}
	}
