...
/// <summary>
/// <see cref="T:System.Double" /> 类型扩展方法类
/// </summary>
public static class DoubleExtensions
{
	...
	public static byte[] GetBytes(this double[] values, int start, int count)
	{
		values.GuardNotNull("values");
	...
	public static byte[] GetBytes(this IEnumerable<double> values)
	{
		values.GuardNotNull("values");
		return values.SelectMany((double x) => x.GetBytes()).ToArray();
	}
	...
	public static byte[] GetRawBytes(this double value)
	{
	...
	/// <returns>Big-Endian 顺序的字节数组</returns>
	/// <exception cref="T:System.ArgumentNullException">当前数组为空。</exception>
	public static byte[] GetRawBytes(this double[] values)
	{
		values.GuardNotNull("values");
	...
	/// <exception cref="T:System.IndexOutOfRangeException">开始转换的索引 <paramref name="start" /> 超出了当前数组的索引范围。</exception>
	/// <exception cref="T:System.ArgumentOutOfRangeException">转换的值数量 <paramref name="count" /> 小于0，或者大于从 <paramref name="start" /> 起剩余的值数量。</exception>
	public static byte[] GetRawBytes(this double[] values, int start, int count)
	{
		values.GuardNotNull("values");
		start.GuardIndexRange(0, values.Length - 1, "start");
		count.GuardBetween(0, values.Length - start, "count");
	...
	/// <returns>Big-Endian 顺序的字节数组</returns>
	/// <exception cref="T:System.ArgumentNullException">当前序列为空。</exception>
	public static byte[] GetRawBytes(this IEnumerable<double> values)
	{
		values.GuardNotNull("values");
		return values.SelectMany((double x) => x.GetRawBytes()).ToArray();
	}
	...
	public static string ToHex(this double value, bool upperCase = false)
	{
	...
	/// <param name="upperCase">是否转换为大写的十六进制字符串形式</param>
	/// <returns>当前数值的十六进制字符串形式</returns>
	public static string ToHex(this double value, string prefix, bool upperCase = false)
	{
