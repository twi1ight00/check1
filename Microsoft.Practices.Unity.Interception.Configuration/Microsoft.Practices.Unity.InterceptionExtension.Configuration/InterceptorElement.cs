...
public class InterceptorElement : InjectionMemberElement
{
	...
	/// </summary>
	[ConfigurationProperty("isDefaultForType", IsRequired = false, DefaultValue = false)]
	public bool IsDefaultForType
	{
		get
		{
			return (bool)base["isDefaultForType"];
		}
		set
		{
			base["isDefaultForType"] = value;
		}
	}
	...
	/// </summary>
	public override string Key => string.Format(CultureInfo.CurrentCulture, "interceptor:{0}", elementNum);
	...
	public override void SerializeContent(XmlWriter writer)
	{
		...
		if (IsDefaultForType)
		{
			writer.WriteAttributeString("isDefaultForType", IsDefaultForType.ToString());
		}
	}
	...
	public override IEnumerable<InjectionMember> GetInjectionMembers(IUnityContainer container, Type fromType, Type toType, string name)
	{
		...
		if (IsDefaultForType)
		{
			return new DefaultInterceptor[1]
			{
				new DefaultInterceptor(interceptorType, Name)
			};
		}
		return new Interceptor[1]
		{
			new Interceptor(interceptorType, Name)
		};
	}
	...
	private Type GuardTypeIsInterceptor(Type resolvedType)
	{
		...
		{
			throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, Resources.ExceptionResolvedTypeNotCompatible, TypeName, resolvedType.FullName, typeof(IInterceptor).FullName));
		}
		return resolvedType;
	}
}
