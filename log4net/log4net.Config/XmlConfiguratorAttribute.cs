...
public class XmlConfiguratorAttribute : ConfiguratorAttribute
{
	...
	private void ConfigureFromFile(Assembly sourceAssembly, ILoggerRepository targetRepository)
	{
		...
		else
		{
			...
			{
				LogLog.Warn(declaringType, "Exception getting ApplicationBaseDirectory. ConfigFile property path [" + m_configFile + "] will be treated as an absolute path.", exception);
			}
	...
	private void ConfigureFromUri(Assembly sourceAssembly, ILoggerRepository targetRepository)
	{
		...
		else
		{
			...
			{
				LogLog.Warn(declaringType, "Exception getting ApplicationBaseDirectory. ConfigFile property path [" + m_configFile + "] will be treated as an absolute URI.", exception);
			}
			uri = ((text3 == null) ? new Uri(m_configFile) : new Uri(new Uri(text3), m_configFile));
		}
		if (!(uri != null))
		{
			return;
