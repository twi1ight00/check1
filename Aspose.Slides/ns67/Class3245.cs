using System;

namespace ns67;

internal class Class3245 : Class3091
{
	private const string string_0 = "adj1";

	private const string string_1 = "adj2";

	public override Enum493 Kind => Enum493.const_27;

	public Class3245(Class3448 xfrm)
		: base(xfrm)
	{
	}

	public override Class3279 vmethod_1()
	{
		double y = Class3089.smethod_0(0.0, base.AdjustValues["adj1"], 50000.0);
		double y2 = Class3089.smethod_0(0.0, base.AdjustValues["adj2"], 50000.0);
		double num = Class3089.smethod_1(Math.Min(base.Transform2D.Extents.Cy, base.Transform2D.Extents.Cx), y, 100000.0);
		double x = Class3089.smethod_2(base.Transform2D.Extents.Cx, 0.0, num);
		double num2 = Class3089.smethod_1(Math.Min(base.Transform2D.Extents.Cy, base.Transform2D.Extents.Cx), y2, 100000.0);
		Class3089.smethod_2(base.Transform2D.Extents.Cx, 0.0, num2);
		double y3 = Class3089.smethod_2(base.Transform2D.Extents.Cy, 0.0, num2);
		double x2 = Class3089.smethod_2(num, 0.0, num2);
		double num3 = Class3089.smethod_1(num, 29289.0, 100000.0);
		double z = Class3089.smethod_1(num2, 29289.0, 100000.0);
		double num4 = Class3089.smethod_4(x2, num3, z);
		double right = Class3089.smethod_2(base.Transform2D.Extents.Cx, 0.0, num4);
		double bottom = Class3089.smethod_2(base.Transform2D.Extents.Cy, 0.0, z);
		Class3279 @class = new Class3279(base.Transform2D);
		@class.method_2(base.Transform2D.Extents.Cx, base.Transform2D.Extents.Cy, Enum492.const_0, extrusionOk: false, stroke: true);
		@class.method_3(num, 0.0);
		@class.method_4(x, 0.0);
		@class.method_5(num, num, 16200000.0, 5400000.0);
		@class.method_4(base.Transform2D.Extents.Cx, y3);
		@class.method_5(num2, num2, 0.0, 5400000.0);
		@class.method_4(num2, base.Transform2D.Extents.Cy);
		@class.method_5(num2, num2, 5400000.0, 5400000.0);
		@class.method_4(0.0, num);
		@class.method_5(num, num, 10800000.0, 5400000.0);
		@class.method_8();
		@class.TextRectangle = new Class3457(num4, num3, right, bottom);
		return @class;
	}

	protected override void vmethod_0(Interface51 values)
	{
		values.Add("adj1", 16667.0);
		values.Add("adj2", 0.0);
	}
}
