...
public static class SqlMapper
{
	...
	private sealed class DapperRow : IDynamicMetaObjectProvider, IDictionary<string, object>, ICollection<KeyValuePair<string, object>>, IEnumerable<KeyValuePair<string, object>>, IEnumerable
	{
		...
		ICollection<string> IDictionary<string, object>.Keys => this.Select((KeyValuePair<string, object> kv) => kv.Key).ToArray();

		ICollection<object> IDictionary<string, object>.Values => this.Select((KeyValuePair<string, object> kv) => kv.Value).ToArray();
		...
		{
			int num = table.IndexOfName(name);
			if (num < 0)
			{
				value = null;
				return false;
	...
	private sealed class DapperRowMetaObject : DynamicMetaObject
	{
		...
		{
			Expression[] parameters = new Expression[1] { Expression.Constant(binder.Name) };
	...
	private sealed class DapperTable
	{
		...
		{
			if (name == null || !fieldNameLookup.TryGetValue(name, out var value))
			{
				return -1;
			}
			return value;
	...
	public class GridReader : IDisposable
	{
		...
		private Task<IEnumerable<T>> ReadAsyncImpl<T>(Type type, bool buffered)
		{
			...
			CacheInfo cacheInfo = GetCacheInfo(typedIdentity, null, addToCache);
			...
			{
				return ReadBufferedAsync<T>(gridIndex, deserializerState.Func, typedIdentity);
			}
			IEnumerable<T> enumerable = ReadDeferred<T>(gridIndex, deserializerState.Func, typedIdentity, type);
			if (buffered)
			{
	...
	public class Identity : IEquatable<Identity>
	{
		...
		{
			if (other != null && gridIndex == other.gridIndex && type == other.type && sql == other.sql)
			{
				CommandType? commandType = this.commandType;
				CommandType? commandType2 = other.commandType;
				if (commandType.GetValueOrDefault() == commandType2.GetValueOrDefault() && commandType.HasValue == commandType2.HasValue && connectionStringComparer.Equals(connectionString, other.connectionString))
				{
	...
	internal class Link<TKey, TValue> where TKey : class
	{
		...
		public static bool TryAdd(ref Link<TKey, TValue> head, TKey key, ref TValue value)
		{
			...
			do
			{
				...
				if (TryGet(link, key, out var value2))
				{
					value = value2;
					return false;
				}
		...
		private struct DeserializerKey : IEquatable<DeserializerKey>
		{
			...
			public DeserializerKey(int hashCode, int startBound, int length, bool returnNullIfFirstMissing, IDataReader reader, bool copyDown)
			{
				...
				if (copyDown)
				{
					...
					{
						names[j] = reader.GetName(i);
						types[j] = reader.GetFieldType(i++);
					}
				}
				else
				{
			...
			public bool Equals(DeserializerKey other)
			{
				...
				{
					string[] array = names;
					string obj = ((array != null) ? array[num] : null) ?? reader?.GetName(startBound + num);
					string[] array2 = other.names;
					if (!(obj != (((array2 != null) ? array2[num] : null) ?? other.reader?.GetName(startBound + num))))
					{
						Type[] array3 = types;
						Type obj2 = ((array3 != null) ? array3[num] : null) ?? reader?.GetFieldType(startBound + num);
						Type[] array4 = other.types;
						if (!(obj2 != (((array4 != null) ? array4[num] : null) ?? other.reader?.GetFieldType(startBound + num))))
						{
							num++;
							continue;
						}
	...
	private static CommandBehavior allowedCommandBehaviors;
	...
	private static async Task<IEnumerable<T>> QueryAsync<T>(this IDbConnection cnn, Type effectiveType, CommandDefinition command)
	{
		...
		using DbCommand cmd = (DbCommand)command.SetupCommand(cnn, info.ParamReader);
		DbDataReader reader = null;
		try
		{
			if (wasClosed)
			{
				await ((DbConnection)cnn).OpenAsync(cancel).ConfigureAwait(continueOnCapturedContext: false);
			}
			...
			if (deserializerState.Func == null || deserializerState.Hash != columnHash)
			{
				DeserializerState deserializerState3 = (info.Deserializer = new DeserializerState(columnHash, GetDeserializer(effectiveType, reader, 0, -1, returnNullIfFirstMissing: false)));
	...
	private static async Task<int> ExecuteMultiImplAsync(IDbConnection cnn, CommandDefinition command, IEnumerable multiExec)
	{
		...
		try
		{
			if (wasClosed)
			{
				await ((DbConnection)cnn).OpenAsync(command.CancellationToken).ConfigureAwait(continueOnCapturedContext: false);
			}
			CacheInfo info = null;
			...
			if ((command.Flags & CommandFlags.Pipelined) != 0)
			{
				...
				try
				{
					foreach (object obj in multiExec)
					{
						...
						{
							isFirst = false;
							cmd2 = (DbCommand)command.SetupCommand(cnn, null);
							masterSql = cmd2.CommandText;
							Identity identity = new Identity(command.CommandText, cmd2.CommandType, cnn, null, obj.GetType(), null);
						...
						info.ParamReader(cmd2, obj);
						Task<int> task = cmd2.ExecuteNonQueryAsync(command.CancellationToken);
						pending.Enqueue(new AsyncExecState(cmd2, task));
						cmd2 = null;
					}
			...
			{
				using DbCommand cmd = (DbCommand)command.SetupCommand(cnn, null);
				foreach (object item in multiExec)
				{
					if (isFirst)
					{
						...
						isFirst = false;
						Identity identity2 = new Identity(command.CommandText, cmd.CommandType, cnn, null, item.GetType(), null);
						info = GetCacheInfo(identity2, item, command.AddToCache);
					}
					else
					{
	...
	private static async Task<int> ExecuteImplAsync(IDbConnection cnn, CommandDefinition command, object param)
	{
		...
		bool wasClosed = cnn.State == ConnectionState.Closed;
		using DbCommand cmd = (DbCommand)command.SetupCommand(cnn, cacheInfo.ParamReader);
		_ = 1;
		try
		{
			if (wasClosed)
	...
	private static async Task<IEnumerable<TReturn>> MultiMapAsync<TFirst, TSecond, TThird, TFourth, TFifth, TSixth, TSeventh, TReturn>(this IDbConnection cnn, CommandDefinition command, Delegate map, string splitOn)
	{
		...
		try
		{
			...
			{
				IEnumerable<TReturn> enumerable2 = enumerable.ToList();
				result = enumerable2;
			}
			return result;
		}
	...
	private static async Task<IEnumerable<TReturn>> MultiMapAsync<TReturn>(this IDbConnection cnn, CommandDefinition command, Type[] types, Func<object[], TReturn> map, string splitOn)
	{
		...
		try
		{
			if (wasClosed)
			{
				await ((DbConnection)cnn).OpenAsync().ConfigureAwait(continueOnCapturedContext: false);
			}
			using DbCommand cmd = (DbCommand)command.SetupCommand(cnn, info.ParamReader);
			using DbDataReader reader = await ExecuteReaderWithFlagsFallbackAsync(cmd, wasClosed, CommandBehavior.SingleResult | CommandBehavior.SequentialAccess, command.CancellationToken).ConfigureAwait(continueOnCapturedContext: false);
	...
	public static Task<GridReader> QueryMultipleAsync(this IDbConnection cnn, string sql, object param = null, IDbTransaction transaction = null, int? commandTimeout = null, CommandType? commandType = null)
	{
		...
		return cnn.QueryMultipleAsync(command);
	}
	...
	private static async Task<IDataReader> ExecuteReaderImplAsync(IDbConnection cnn, CommandDefinition command)
	{
		...
		try
		{
			cmd = (DbCommand)command.SetupCommand(cnn, parameterReader);
			if (wasClosed)
			{
				await ((DbConnection)cnn).OpenAsync(command.CancellationToken).ConfigureAwait(continueOnCapturedContext: false);
			}
			DbDataReader result = await ExecuteReaderWithFlagsFallbackAsync(cmd, wasClosed, CommandBehavior.Default, command.CancellationToken).ConfigureAwait(continueOnCapturedContext: false);
			wasClosed = false;
			return result;
		}
		finally
	...
	private static void CollectCacheGarbage()
	{
		try
		{
			foreach (KeyValuePair<Identity, CacheInfo> item in _queryCache)
			{
				...
				{
					_queryCache.TryRemove(item.Key, out var _);
				}
			}
		}
		finally
	...
	{
		IEnumerable<Tuple<string, string, int>> enumerable = _queryCache.Select((KeyValuePair<Identity, CacheInfo> pair) => Tuple.Create(pair.Key.connectionString, pair.Key.sql, pair.Value.GetHitCount()));
		if (ignoreHitCountAbove < int.MaxValue)
		{
			enumerable = enumerable.Where((Tuple<string, string, int> tuple) => tuple.Item3 <= ignoreHitCountAbove);
		}
		return enumerable;
	}
	...
	public static IEnumerable<Tuple<int, int>> GetHashCollissions()
	{
		...
		{
			if (!dictionary.TryGetValue(key.hashCode, out var value))
			{
				dictionary.Add(key.hashCode, 1);
			}
			else
			{
				dictionary[key.hashCode] = value + 1;
			}
		}
	...
	static SqlMapper()
	{
		...
		allowedCommandBehaviors = (CommandBehavior)(-1);
		smellsLikeOleDb = new Regex("(?<![a-z0-9@_])[?@:](?![a-z0-9@_])", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Compiled | RegexOptions.CultureInvariant);
		literalTokens = new Regex("(?<![a-z0-9_])\\{=([a-z0-9_]+)\\}", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Compiled | RegexOptions.CultureInvariant);
		pseudoPositional = new Regex("\\?([a-z_][a-z0-9_]*)\\?", RegexOptions.IgnoreCase | RegexOptions.Compiled | RegexOptions.CultureInvariant);
		...
		toStrings = new Type[12]
		{
			...
			typeof(double),
			typeof(decimal)
		}.ToDictionary((Type x) => TypeExtensions.GetTypeCode(x), (Type x) => x.GetPublicInstanceMethod("ToString", new Type[1] { typeof(IFormatProvider) }));
		...
		getItem = (from p in typeof(IDataRecord).GetProperties(BindingFlags.Instance | BindingFlags.Public)
			where p.GetIndexParameters().Any() && p.GetIndexParameters()[0].ParameterType == typeof(int)
	...
	public static void AddTypeMap(Type type, DbType dbType)
	{
		...
		if (!dictionary.TryGetValue(type, out var value) || value != dbType)
		{
			typeMap = new Dictionary<Type, DbType>(dictionary) { [type] = dbType };
		}
	}
	...
	public static void AddTypeHandlerImpl(Type type, ITypeHandler handler, bool clone)
	{
		...
		if (dictionary.TryGetValue(type, out var value) && handler == value)
		{
			return;
		}
		Dictionary<Type, ITypeHandler> dictionary2 = (clone ? new Dictionary<Type, ITypeHandler>(dictionary) : dictionary);
		typeof(TypeHandlerCache<>).MakeGenericType(type).GetMethod("SetHandler", BindingFlags.Static | BindingFlags.NonPublic).Invoke(null, new object[1] { handler });
		if (type2 != null)
		{
			typeof(TypeHandlerCache<>).MakeGenericType(type2).GetMethod("SetHandler", BindingFlags.Static | BindingFlags.NonPublic).Invoke(null, new object[1] { handler });
		}
		if (handler == null)
		{
	...
	private static IEnumerable<TReturn> MultiMapImpl<TFirst, TSecond, TThird, TFourth, TFifth, TSixth, TSeventh, TReturn>(this IDbConnection cnn, CommandDefinition command, Delegate map, string splitOn, IDataReader reader, Identity identity, bool finalize)
	{
		...
		try
		{
			...
			DeserializerState deserializerState2 = (deserializerState = cacheInfo.Deserializer);
			Func<IDataReader, object>[] otherDeserializers;
			if (deserializerState2.Func == null || (otherDeserializers = cacheInfo.OtherDeserializers) == null || columnHash != deserializerState.Hash)
			{
				...
				DeserializerState deserializerState4 = (cacheInfo.Deserializer = new DeserializerState(columnHash, array[0]));
				deserializerState = deserializerState4;
	...
	{
		if (allowedCommandBehaviors == (CommandBehavior)(-1) && (behavior & (CommandBehavior.SingleResult | CommandBehavior.SingleRow)) != 0 && (ex.Message.Contains("SingleResult") || ex.Message.Contains("SingleRow")))
		{
	...
	private static Func<IDataReader, TReturn> GenerateMapper<TFirst, TSecond, TThird, TFourth, TFifth, TSixth, TSeventh, TReturn>(Func<IDataReader, object> deserializer, Func<IDataReader, object>[] otherDeserializers, object map)
	{
		...
		{
			1 => (IDataReader r) => ((Func<TFirst, TSecond, TReturn>)map)((TFirst)deserializer(r), (TSecond)otherDeserializers[0](r)), 
			2 => (IDataReader r) => ((Func<TFirst, TSecond, TThird, TReturn>)map)((TFirst)deserializer(r), (TSecond)otherDeserializers[0](r), (TThird)otherDeserializers[1](r)), 
			3 => (IDataReader r) => ((Func<TFirst, TSecond, TThird, TFourth, TReturn>)map)((TFirst)deserializer(r), (TSecond)otherDeserializers[0](r), (TThird)otherDeserializers[1](r), (TFourth)otherDeserializers[2](r)), 
			4 => (IDataReader r) => ((Func<TFirst, TSecond, TThird, TFourth, TFifth, TReturn>)map)((TFirst)deserializer(r), (TSecond)otherDeserializers[0](r), (TThird)otherDeserializers[1](r), (TFourth)otherDeserializers[2](r), (TFifth)otherDeserializers[3](r)), 
			5 => (IDataReader r) => ((Func<TFirst, TSecond, TThird, TFourth, TFifth, TSixth, TReturn>)map)((TFirst)deserializer(r), (TSecond)otherDeserializers[0](r), (TThird)otherDeserializers[1](r), (TFourth)otherDeserializers[2](r), (TFifth)otherDeserializers[3](r), (TSixth)otherDeserializers[4](r)), 
			6 => (IDataReader r) => ((Func<TFirst, TSecond, TThird, TFourth, TFifth, TSixth, TSeventh, TReturn>)map)((TFirst)deserializer(r), (TSecond)otherDeserializers[0](r), (TThird)otherDeserializers[1](r), (TFourth)otherDeserializers[2](r), (TFifth)otherDeserializers[3](r), (TSixth)otherDeserializers[4](r), (TSeventh)otherDeserializers[5](r)), 
	...
	{
		if (!TryGetQueryCache(identity, out var value))
		{
	...
	private static Func<IDataReader, object> GetDeserializer(Type type, IDataReader reader, int startBound, int length, bool returnNullIfFirstMissing)
	{
		...
		{
			if (typeHandlers.TryGetValue(type, out var value))
			{
	...
	public static void PackListParameters(IDbCommand command, string namePrefix, object value)
	{
		...
		if (list != null && !flag3)
		{
			...
			foreach (object item in list)
			{
				if (++count == 1)
				{
					...
					if (!flag2)
					{
						dbType = LookupDbType(item.GetType(), "", demand: true, out var _);
					}
				}
				string text = namePrefix + count;
			...
			if (Settings.PadListExpansions && !flag2 && obj != null)
			{
				...
				for (int i = 0; i < listPaddingExtraCount; i++)
				{
					...
					dbDataParameter3.ParameterName = namePrefix + count;
					if (flag)
					{
		...
		if (count == 0)
		{
			command.CommandText = Regex.Replace(command.CommandText, inListRegex, delegate(Match match)
			{
				...
				return match.Groups[2].Success ? match.Value : ("(SELECT " + value4 + " WHERE 1 = 0)");
	...
	private static bool TryStringSplit(ref IEnumerable list, int splitAt, string namePrefix, IDbCommand command, bool byPosition)
	{
		...
		{
			return TryStringSplit(ref list, splitAt, namePrefix, command, "int", byPosition, delegate(StringBuilder sb, int i)
			{
		...
		{
			return TryStringSplit(ref list, splitAt, namePrefix, command, "bigint", byPosition, delegate(StringBuilder sb, long i)
			{
				sb.Append(i.ToString(CultureInfo.InvariantCulture));
			});
		}
		...
		{
			return TryStringSplit(ref list, splitAt, namePrefix, command, "smallint", byPosition, delegate(StringBuilder sb, short i)
			{
				sb.Append(i.ToString(CultureInfo.InvariantCulture));
			});
		}
		...
		{
			return TryStringSplit(ref list, splitAt, namePrefix, command, "tinyint", byPosition, delegate(StringBuilder sb, byte i)
			{
				sb.Append(i.ToString(CultureInfo.InvariantCulture));
			});
		}
	...
	private static bool TryStringSplit<T>(ref IEnumerable list, int splitAt, string namePrefix, IDbCommand command, string colType, bool byPosition, Action<StringBuilder, T> append)
	{
		...
		if (collection == null)
		{
			collection = (ICollection<T>)(list = ((IEnumerable<T>)list).ToList());
		}
	...
	{
		return parameters.Where((PropertyInfo p) => Regex.IsMatch(sql, "[?@:]" + p.Name + "([^a-z0-9_]+|$)", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.CultureInvariant));
	}
	...
	internal static Action<IDbCommand, object> CreateParamInfoGenerator(Identity identity, bool checkForDuplicates, bool removeUnused, IList<LiteralToken> literals)
	{
		...
		if (constructors.Length == 1 && array.Length == (parameters = constructors[0].GetParameters()).Length)
		{
			...
			else
			{
				...
				if (dictionary.Count == array.Length)
				{
					...
					{
						if (!dictionary.TryGetValue(array[k].Name, out var value))
						{
							flag4 = false;
							break;
						}
		...
		if (enumerable == null)
		{
			enumerable = array.OrderBy((PropertyInfo x) => x.Name);
		}
		if (flag)
		{
		...
		if (literals.Count != 0 && array != null)
		{
			...
			foreach (LiteralToken literal in literals)
			{
				...
				iLGenerator.Emit(OpCodes.Ldloc_0);
				iLGenerator.EmitCall(opcode, propertyInfo3.GetGetMethod(), null);
				Type propertyType = propertyInfo3.PropertyType;
	...
	{
		if (!toStrings.TryGetValue(typeCode, out var value))
		{
			return null;
		}
		return value;
	...
	private static Func<IDataReader, object> GetStructDeserializer(Type type, Type effectiveType, int index)
	{
		...
		if (effectiveType.IsEnum())
		{
			return delegate(IDataReader r)
			{
				...
				return (!(obj is DBNull)) ? Enum.ToObject(effectiveType, obj) : null;
			};
		}
		if (typeHandlers.TryGetValue(type, out var handler))
		{
			return delegate(IDataReader r)
			{
				...
				return (!(value2 is DBNull)) ? handler.Parse(type, value2) : null;
			};
		}
		return delegate(IDataReader r)
		{
	...
	private static T Parse<T>(object value)
	{
		...
		if (typeHandlers.TryGetValue(typeFromHandle, out var value2))
		{
	...
	private static LocalBuilder GetTempLocal(ILGenerator il, ref Dictionary<Type, LocalBuilder> locals, Type type, bool initAndLoad)
	{
		...
		if (!locals.TryGetValue(type, out var value))
		{
	...
	private static Func<IDataReader, object> GetTypeDeserializerImpl(Type type, IDataReader reader, int startBound = 0, int length = -1, bool returnNullIfFirstMissing = false)
	{
		...
		DynamicMethod dynamicMethod = new DynamicMethod("Deserialize" + Guid.NewGuid().ToString(), type2, new Type[1] { typeof(IDataReader) }, type, skipVisibility: true);
		ILGenerator iLGenerator = dynamicMethod.GetILGenerator();
		iLGenerator.DeclareLocal(typeof(int));
		...
		List<IMemberMap> list = ((specializedConstructor != null) ? names.Select((string n) => typeMap.GetConstructorParameter(specializedConstructor, n)) : names.Select((string n) => typeMap.GetMember(n))).ToList();
		bool flag2 = true;
		Label label = iLGenerator.DefineLabel();
		int num2 = -1;
		int localIndex = iLGenerator.DeclareLocal(typeof(object)).LocalIndex;
		bool applyNullValues = Settings.ApplyNullValues;
		foreach (IMemberMap item in list)
		{
			if (item != null)
			{
				...
				else
				{
					...
					if (type3.IsEnum())
					{
						...
						if (underlyingType != null)
						{
							iLGenerator.Emit(OpCodes.Newobj, memberType.GetConstructor(new Type[1] { underlyingType }));
						}
					}
					else if (memberType.FullName == "System.Data.Linq.Binary")
					{
						iLGenerator.Emit(OpCodes.Unbox_Any, typeof(byte[]));
						iLGenerator.Emit(OpCodes.Newobj, memberType.GetConstructor(new Type[1] { typeof(byte[]) }));
					}
					else
					{
						...
						else
						{
							...
							if (underlyingType != null)
							{
								iLGenerator.Emit(OpCodes.Newobj, type3.GetConstructor(new Type[1] { underlyingType }));
							}
						}
					}
				}
	...
	private static void FlexibleConvertBoxedFromHeadOfStack(ILGenerator il, Type from, Type to, Type via)
	{
		...
		MethodInfo @operator;
		if ((@operator = GetOperator(from, to)) != null)
		{
	...
	public static Func<IDataReader, T> GetRowParser<T>(this IDataReader reader, Type concreteType = null, int startIndex = 0, int length = -1, bool returnNullIfFirstMissing = false)
	{
		...
		return (Func<IDataReader, T>)(object)func;
	}
}
