...
public class MimeEntity
{
	...
	public MediaType_enum ContentType
	{
		get
		{
			...
			{
				string value = new ParametizedHeaderField(m_pHeader.GetFirst("Content-Type:")).Value;
				return MimeUtils.ParseMediaType(value);
			}
	...
	public string ContentType_CharSet
	{
		...
		set
		{
			...
			if (DataEncoded != null)
			{
				...
				try
				{
					Encoding.GetEncoding(contentType_Boundary);
				}
				catch
				{
					throw new Exception("Data can't be converted because current charset '" + contentType_Boundary + "' isn't supported !");
				}
				try
				{
	...
	public string ContentDisposition_FileName
	{
		get
		{
			if (m_pHeader.Contains("Content-Disposition:"))
			{
				...
				{
					return parametizedHeaderField.Parameters["filename"];
				}
				return null;
			}
			return null;
		...
		set
		{
			...
			if (parametizedHeaderField.Parameters.Contains("filename"))
			{
				parametizedHeaderField.Parameters["filename"] = value;
			}
			else
			{
				parametizedHeaderField.Parameters.Add("filename", value);
			}
		}
	}
	...
	public string DataText
	{
		...
		set
		{
			...
			try
			{
				Data = Encoding.GetEncoding(contentType_CharSet).GetBytes(value);
			}
			catch
			{
	...
	internal bool Parse(Stream stream, string toBoundary)
	{
		...
		if ((ContentType & MediaType_enum.Multipart) != 0)
		{
			...
			if (contentType_Boundary != null)
			{
				...
				byte[] array;
				for (array = streamLineReader.ReadLine(); array != null; array = streamLineReader.ReadLine())
				{
					string @string = Encoding.Default.GetString(array);
					if (@string.StartsWith("--" + contentType_Boundary))
					{
						break;
					}
				}
				...
				if (toBoundary != null && toBoundary.Length > 0)
				{
					...
					{
						string @string = Encoding.Default.GetString(array);
						if (@string.StartsWith("--" + toBoundary))
						{
							break;
						}
					}
					...
					if (Encoding.Default.GetString(array).EndsWith(toBoundary + "--"))
					{
						return false;
					}
					return true;
		...
		else
		{
			if (toBoundary != null && toBoundary.Length > 0)
			{
				...
				while (array != null)
				{
					...
					array = streamLineReader.ReadLine();
				}
				if (array == null)
				{
