...
public class DynamicParameters : SqlMapper.IDynamicParameters, SqlMapper.IParameterLookup, SqlMapper.IParameterCallbacks
{
	...
	object SqlMapper.IParameterLookup.this[string member]
	{
		get
		{
			if (!parameters.TryGetValue(member, out var value))
			{
				return null;
			}
			return value.Value;
	...
	public IEnumerable<string> ParameterNames => parameters.Select((KeyValuePair<string, ParamInfo> p) => p.Key);
	...
	public DynamicParameters Output<T>(T target, Expression<Func<T, object>> expression, DbType? dbType = null, int? size = null)
	{
		...
		if (setter == null)
		{
			...
			MethodInfo meth = GetType().GetMethod("Get", new Type[1] { typeof(string) }).MakeGenericMethod(lastMemberAccess.Type);
		...
		(outputCallbacks ?? (outputCallbacks = new List<Action>())).Add(delegate
		{
			...
			if (parameters.TryGetValue(dynamicParamName, out var value))
			{
				ParamInfo paramInfo = value;
				ParameterDirection parameterDirection2 = (value.AttachedParam.Direction = ParameterDirection.InputOutput);
				paramInfo.ParameterDirection = parameterDirection2;
				if (value.AttachedParam.Size == 0)
				{
					ParamInfo paramInfo2 = value;
					int value2 = (value.AttachedParam.Size = num);
					paramInfo2.Size = value2;
				}
			}
			else
			{
				dbType = ((!dbType.HasValue) ? new DbType?(SqlMapper.LookupDbType(type, type?.Name, demand: true, out var _)) : dbType);
	...
	{
		foreach (ParamInfo item in parameters.Select((KeyValuePair<string, ParamInfo> p) => p.Value))
		{
