using System;

namespace ns67;

internal class Class3198 : Class3091
{
	private const string string_0 = "adj";

	public override Enum493 Kind => Enum493.const_127;

	public Class3198(Class3448 xfrm)
		: base(xfrm)
	{
	}

	public override Class3279 vmethod_1()
	{
		double y = Class3089.smethod_0(0.0, base.AdjustValues["adj"], 25000.0);
		double num = Class3089.smethod_1(Math.Min(base.Transform2D.Extents.Cy, base.Transform2D.Extents.Cx), y, 100000.0);
		double num2 = Class3089.smethod_1(num, 1.0, 2.0);
		double num3 = Class3089.smethod_1(num, 1.0, 4.0);
		double y2 = Class3089.smethod_2(num, num2, 0.0);
		double y3 = Class3089.smethod_2(num, num, 0.0);
		double num4 = Class3089.smethod_2(base.Transform2D.Extents.Cy, 0.0, num);
		double y4 = Class3089.smethod_2(base.Transform2D.Extents.Cy, 0.0, num2);
		double y5 = Class3089.smethod_2(num4, 0.0, num2);
		double x = Class3089.smethod_2(base.Transform2D.Extents.Cx, 0.0, num);
		double num5 = Class3089.smethod_2(base.Transform2D.Extents.Cx, 0.0, num2);
		Class3279 @class = new Class3279(base.Transform2D);
		@class.method_2(base.Transform2D.Extents.Cx, base.Transform2D.Extents.Cy, Enum492.const_0, extrusionOk: false, stroke: false);
		@class.method_3(base.Transform2D.Extents.Cx, num2);
		@class.method_5(num2, num2, 0.0, 5400000.0);
		@class.method_4(num5, num2);
		@class.method_5(num3, num3, 0.0, 10800000.0);
		@class.method_4(x, num);
		@class.method_4(num2, num);
		@class.method_5(num2, num2, 16200000.0, -5400000.0);
		@class.method_4(0.0, y4);
		@class.method_5(num2, num2, 10800000.0, -10800000.0);
		@class.method_4(num, num4);
		@class.method_4(num5, num4);
		@class.method_5(num2, num2, 5400000.0, -5400000.0);
		@class.method_8();
		@class.method_3(num2, y3);
		@class.method_5(num2, num2, 5400000.0, -5400000.0);
		@class.method_5(num3, num3, 0.0, -10800000.0);
		@class.method_8();
		@class.method_2(base.Transform2D.Extents.Cx, base.Transform2D.Extents.Cy, Enum492.const_2, extrusionOk: false, stroke: false);
		@class.method_3(num2, y3);
		@class.method_5(num2, num2, 5400000.0, -5400000.0);
		@class.method_5(num3, num3, 0.0, -10800000.0);
		@class.method_8();
		@class.method_3(num5, num);
		@class.method_5(num2, num2, 5400000.0, -16200000.0);
		@class.method_5(num3, num3, 10800000.0, -10800000.0);
		@class.method_8();
		@class.method_2(base.Transform2D.Extents.Cx, base.Transform2D.Extents.Cy, Enum492.const_5, extrusionOk: false, stroke: true);
		@class.method_3(0.0, y2);
		@class.method_5(num2, num2, 10800000.0, 5400000.0);
		@class.method_4(x, num);
		@class.method_4(x, num2);
		@class.method_5(num2, num2, 10800000.0, 10800000.0);
		@class.method_4(base.Transform2D.Extents.Cx, y5);
		@class.method_5(num2, num2, 0.0, 5400000.0);
		@class.method_4(num, num4);
		@class.method_4(num, y4);
		@class.method_5(num2, num2, 0.0, 10800000.0);
		@class.method_8();
		@class.method_3(x, num);
		@class.method_4(num5, num);
		@class.method_5(num2, num2, 5400000.0, -5400000.0);
		@class.method_3(num5, num);
		@class.method_4(num5, num2);
		@class.method_5(num3, num3, 0.0, 10800000.0);
		@class.method_3(num2, y3);
		@class.method_4(num2, y2);
		@class.method_5(num3, num3, 10800000.0, 10800000.0);
		@class.method_5(num2, num2, 0.0, 10800000.0);
		@class.method_3(num, y2);
		@class.method_4(num, num4);
		@class.TextRectangle = new Class3457(num, num, num5, num4);
		return @class;
	}

	protected override void vmethod_0(Interface51 values)
	{
		values.Add("adj", 12500.0);
	}
}
