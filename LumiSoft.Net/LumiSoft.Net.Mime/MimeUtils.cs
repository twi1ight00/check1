...
public class MimeUtils
{
	public static DateTime ParseDate(string date)
	{
		...
		if (stringReader.Available > 0)
		{
			...
			if (stringReader.StartsWith(":"))
			{
				...
				try
				{
					num6 = Convert.ToInt32(stringReader.ReadWord(unQuote: true, new char[1] { ' ' }, removeWordTerminator: true));
				}
				catch
				{
			...
			{
				string text = stringReader.SourceString.Replace(":", "");
				if (text.StartsWith("+") || text.StartsWith("-"))
				{
					...
					try
					{
						int num8 = Convert.ToInt32(text.Substring(0, 2));
						int num9 = Convert.ToInt32(text.Substring(2));
	...
	public static string EncodeHeaderField(string text)
	{
		...
		if (text.IndexOf("\"") > -1)
		{
			...
			return Core.CanonicalEncode(text2, "utf-8");
		}
