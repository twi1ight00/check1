...
public class InterceptionBehaviorElement : InjectionMemberElement
{
	...
	public override IEnumerable<InjectionMember> GetInjectionMembers(IUnityContainer container, Type fromType, Type toType, string name)
	{
		...
		GuardBehaviorType(type);
		if (IsDefaultForType)
		{
			return new DefaultInterceptionBehavior[1]
			{
				new DefaultInterceptionBehavior(type, Name)
			};
		}
		return new InterceptionBehavior[1]
		{
			new InterceptionBehavior(type, Name)
		};
	}
	...
	private void GuardBehaviorType(Type resolvedType)
	{
		...
		{
			throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, Resources.ExceptionResolvedTypeNotCompatible, TypeName, resolvedType.FullName, typeof(IInterceptionBehavior).FullName));
		}
	}
}
