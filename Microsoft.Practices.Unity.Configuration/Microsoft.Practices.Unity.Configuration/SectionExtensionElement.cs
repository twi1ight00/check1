...
public class SectionExtensionElement : DeserializableConfigurationElement
{
	...
	private void GuardIsValidExtensionType(Type extensionType)
	{
		...
		{
			throw new ConfigurationErrorsException(string.Format(CultureInfo.CurrentCulture, Resources.ExtensionTypeNotFound, TypeName));
		}
		if (!typeof(SectionExtension).IsAssignableFrom(extensionType))
		{
			throw new ConfigurationErrorsException(string.Format(CultureInfo.CurrentCulture, Resources.ExtensionTypeNotValid, TypeName));
		}
	}
	...
	private Type GetExtensionObjectType()
	{
		...
		GuardIsValidExtensionType(type);
		return type;
	}
}
