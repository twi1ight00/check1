...
public sealed class DefaultTypeMap : SqlMapper.ITypeMap
{
	...
	{
		foreach (ConstructorInfo item in from c in _type.GetConstructors(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
			orderby (!c.IsPublic) ? ((!c.IsPrivate) ? 1 : 2) : 0, c.GetParameters().Length
			select c)
		{
	...
	{
		PropertyInfo propertyInfo = Properties.FirstOrDefault((PropertyInfo p) => string.Equals(p.Name, columnName, StringComparison.Ordinal)) ?? Properties.FirstOrDefault((PropertyInfo p) => string.Equals(p.Name, columnName, StringComparison.OrdinalIgnoreCase));
		if (propertyInfo == null && MatchNamesWithUnderscores)
		{
			propertyInfo = Properties.FirstOrDefault((PropertyInfo p) => string.Equals(p.Name, columnName.Replace("_", ""), StringComparison.Ordinal)) ?? Properties.FirstOrDefault((PropertyInfo p) => string.Equals(p.Name, columnName.Replace("_", ""), StringComparison.OrdinalIgnoreCase));
		}
		...
		FieldInfo fieldInfo = _fields.FirstOrDefault((FieldInfo p) => string.Equals(p.Name, columnName, StringComparison.Ordinal)) ?? _fields.FirstOrDefault((FieldInfo p) => string.Equals(p.Name, backingFieldName, StringComparison.Ordinal)) ?? _fields.FirstOrDefault((FieldInfo p) => string.Equals(p.Name, columnName, StringComparison.OrdinalIgnoreCase)) ?? _fields.FirstOrDefault((FieldInfo p) => string.Equals(p.Name, backingFieldName, StringComparison.OrdinalIgnoreCase));
		if (fieldInfo == null && MatchNamesWithUnderscores)
		{
			...
			fieldInfo = _fields.FirstOrDefault((FieldInfo p) => string.Equals(p.Name, effectiveColumnName, StringComparison.Ordinal)) ?? _fields.FirstOrDefault((FieldInfo p) => string.Equals(p.Name, backingFieldName, StringComparison.Ordinal)) ?? _fields.FirstOrDefault((FieldInfo p) => string.Equals(p.Name, effectiveColumnName, StringComparison.OrdinalIgnoreCase)) ?? _fields.FirstOrDefault((FieldInfo p) => string.Equals(p.Name, backingFieldName, StringComparison.OrdinalIgnoreCase));
		}
		if (fieldInfo != null)
		{
