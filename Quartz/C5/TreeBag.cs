...
internal class TreeBag<T> : SequencedBase<T>, IIndexedSorted<T>, IIndexed<T>, IPersistentSorted<T>, ISorted<T>, ISequenced<T>, ICollection<T>, IExtensible<T>, ICloneable, System.Collections.Generic.ICollection<T>, IDirectedCollectionValue<T>, ICollectionValue<T>, IShowable, IFormattable, IDirectedEnumerable<T>, IEnumerable<T>, IEnumerable, IDisposable
{
	...
	private class Multiplicities : CollectionValueBase<KeyValuePair<T, int>>, ICollectionValue<KeyValuePair<T, int>>, IEnumerable<KeyValuePair<T, int>>, IEnumerable, IShowable, IFormattable
	{
		...
		public override int Count
		{
			...
			{
				int num = 0;
				using IEnumerator<KeyValuePair<T, int>> enumerator = GetEnumerator();
		...
		{
			return getEnumerator(treebag.root, origstamp);
		}
		...
		private IEnumerator<KeyValuePair<T, int>> getEnumerator(Node node, int origstamp)
		{
			...
			if (node.left != null)
			{
				IEnumerator<KeyValuePair<T, int>> child2 = getEnumerator(node.left, origstamp);
	...
	private class Interval : DirectedCollectionValueBase<T>, IDirectedCollectionValue<T>, ICollectionValue<T>, IShowable, IFormattable, IDirectedEnumerable<T>, IEnumerable<T>, IEnumerable
	{
		...
		public override IEnumerator<T> GetEnumerator()
		{
			...
			while (true)
			{
				int num6 = ((cursor.left != null) ? cursor.left.size : 0);
				if (j > num6)
				{
					if (j - num6 < cursor.items)
					{
						togo = j - num6 + 1;
						break;
					}
	...
	private bool addIterative(T item, ref T founditem, bool update, out bool wasfound)
	{
		...
		while (cursor.red)
		{
			...
			cursor.size = (node3.size = ((node3.left != null) ? node3.left.size : 0) + ((node3.right != null) ? node3.right.size : 0) + node3.items);
			node3 = cursor.left;
			node3.size = ((node3.left != null) ? node3.left.size : 0) + ((node3.right != null) ? node3.right.size : 0) + node3.items;
			cursor.size += node3.size + cursor.items;
			if (num == 0)
			{
				root = cursor;
				return true;
	...
	private static Node maketreer(ref Node rest, int blackheight, int maxred, int red)
	{
		...
		node3.size = node3.items + node3.left.size + node3.right.size;
		return node3;
	}
	...
	private void addSorted<U>(IEnumerable<U> items, bool safe, bool raise) where U : T
	{
		...
		while (enumerator.MoveNext())
		{
			T val = (T)(object)enumerator.Current;
			int num3 = comparer.Compare(x, val);
			if (num3 > 0)
			{
		...
		root = maketreer(ref rest, num4, num6, num5);
		blackdepth = num4;
		size = num;
		size += num2;
		if (!raise)
	...
	private bool removeIterative(ref T item, bool all, out int wasRemoved)
	{
		...
		while (true)
		{
			int num2 = comparer.Compare(cursor.item, item);
			if (num2 == 0)
			{
				item = cursor.item;
				...
				return removeIterativePhase2(cursor, num);
			}
			Node node = ((num2 > 0) ? cursor.left : cursor.right);
			if (node == null)
			{
				break;
			}
		...
		if (flag)
		{
			...
			if (cursor2.red)
			{
				...
				if (node3 != null && node3.red)
				{
					...
					cursor3.size = cursor.size - cursor.items - node2.size;
					cursor4.size = cursor3.size - cursor3.items - node3.size;
				}
				else if (cursor5 != null && cursor5.red)
				{
					...
					cursor2.red = false;
					cursor.size = cursor4.size;
					cursor4.size = cursor4.items + ((cursor4.left != null) ? cursor4.left.size : 0) + ((cursor4.right != null) ? cursor4.right.size : 0);
					cursor3.size = cursor3.items + ((cursor3.left != null) ? cursor3.left.size : 0) + ((cursor3.right != null) ? cursor3.right.size : 0);
					cursor5.size = cursor5.items + cursor4.size + cursor3.size;
				}
				else
				{
					...
					cursor3.red = true;
					cursor.size = cursor4.size;
					cursor4.size -= node2.size + cursor.items;
				}
			}
			...
			else
			{
				...
				cursor.size = cursor4.size;
				cursor4.size = cursor4.items + ((cursor4.left != null) ? cursor4.left.size : 0) + ((cursor4.right != null) ? cursor4.right.size : 0);
				cursor2.size = cursor2.items + ((cursor2.left != null) ? cursor2.left.size : 0) + ((cursor2.right != null) ? cursor2.right.size : 0);
			}
			if (level == 0)
			{
				root = cursor;
	...
	public void RetainAll<U>(IEnumerable<U> items) where U : T
	{
		...
		updatecheck();
		TreeBag<T> treeBag = (TreeBag<T>)MemberwiseClone();
		T j = default(T);
		treeBag.clear();
		foreach (U item2 in items)
		{
			T item = (T)(object)item2;
			if (ContainsCount(item) > treeBag.ContainsCount(item))
			{
				treeBag.add(item, ref j);
			}
		}
		if (size == treeBag.size)
		{
			return;
		}
		...
		if ((ActiveEvents & EventTypeEnum.Removed) != 0)
		{
			...
			foreach (KeyValuePair<T, int> item3 in treeBag.ItemMultiplicities())
			{
				while (enumerator2.MoveNext())
				{
					if (comparer.Compare(enumerator2.Current.Key, item3.Key) == 0)
					{
						int num = enumerator2.Current.Value - item3.Value;
						if (num > 0)
						{
							circularQueue.Enqueue(new KeyValuePair<T, int>(item3.Key, num));
						}
						break;
					}
	...
	public IIndexedSorted<T> FindAll(Fun<T, bool> filter)
	{
		...
		TreeBag<T> treeBag = new TreeBag<T>(comparer);
		...
		while (enumerator.MoveNext())
		{
			...
			if (node != null && comparer.Compare(current, node.item) == 0)
			{
				node.items++;
				num2++;
			}
		...
		treeBag.root = maketreer(ref rest, num3, num5, num4);
		treeBag.blackdepth = num3;
		treeBag.size = num;
		treeBag.size += num2;
		return treeBag;
	}
	...
	public IIndexedSorted<V> Map<V>(Fun<T, V> mapper, IComparer<V> c)
	{
		...
		IEnumerator<T> enumerator = GetEnumerator();
		TreeBag<V>.Node rest = null;
		TreeBag<V>.Node node = null;
		V x = default(V);
		int num = 0;
		T y = default(T);
		while (enumerator.MoveNext())
		{
			T current = enumerator.Current;
			if (node != null && comparer.Compare(current, y) == 0)
			{
				node.items++;
				continue;
			}
		...
		treeBag.root = TreeBag<V>.maketreer(ref rest, num3, num5, num4);
		treeBag.blackdepth = num3;
		treeBag.size = size;
		return treeBag;
	}
	...
	public void RemoveAllCopies(T item)
	{
		...
		updatecheck();
		if (removeIterative(ref item, all: true, out var wasRemoved) && ActiveEvents != 0)
		{
			raiseForRemove(item, wasRemoved);
		}
	}
	...
	private int indexOf(T item, out int upper)
	{
		...
		while (node != null)
		{
			...
			if (num2 == 0)
			{
				upper = num + num3 + node.items - 1;
				return num + num3;
			}
			num = num + node.items + num3;
			node = node.right;
		}
		upper = ~num;
		return ~num;
	...
	public void Dispose()
	{
		...
		if (snapList != null)
		{
			...
			{
				if (prev.Tree.Target is TreeBag<T> treeBag)
				{
					treeBag.snapDispose();
				}
			}
	...
	public ISorted<T> Snapshot()
	{
		...
		TreeBag<T> treeBag = (TreeBag<T>)MemberwiseClone();
		SnapRef snapRef = new SnapRef(treeBag);
		treeBag.isReadOnlyBase = true;
		treeBag.isSnapShot = true;
		treeBag.snapList = snapRef;
	...
	private void minidump(Node n, string space)
	{
		if (n != null)
		{
			...
			Console.WriteLine(string.Format("{0} {4} (size={1}, items={8}, h={2}, gen={3}, id={6}){7}", space + n.item, n.size, 0, n.generation, n.red ? "RED" : "BLACK", 0, 0, (n.lastgeneration == -1) ? "" : string.Format(" [extra: lg={0}, c={1}, i={2}]", n.lastgeneration, n.leftnode ? "L" : "R", (n.oldref == null) ? "()" : string.Concat(n.oldref.item)), n.items));
	...
	private bool rbminicheck(Node n, bool redp, TextWriter o, out T min, out T max, out int blackheight)
	{
		...
		bool b = n.size == ((n.left != null) ? n.left.size : 0) + ((n.right != null) ? n.right.size : 0) + n.items;
	...
	{
		TreeBag<T> treeBag = new TreeBag<T>(comparer, EqualityComparer);
		treeBag.AddSorted(this);
		return treeBag;
	}
}
