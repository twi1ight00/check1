...
public class AdoNetAppender : BufferingAppenderSkeleton
{
	...
	{
		base.OnClose();
		DisposeCommand(ignoreException: false);
		DiposeConnection();
	}
	...
	protected virtual string ResolveConnectionString(out string connectionStringContext)
	{
		...
		{
			ConnectionStringSettings connectionStringSettings = ConfigurationManager.ConnectionStrings[m_connectionStringName];
		...
		if (m_appSettingsKey != null && m_appSettingsKey.Length > 0)
		{
			...
			string appSetting = SystemInfo.GetAppSetting(m_appSettingsKey);
	...
	private void InitializeDatabaseCommand()
	{
		...
		catch (Exception e)
		{
			...
			DisposeCommand(ignoreException: true);
		}
		if (m_dbCommand != null)
		{
			try
	...
	private void InitializeDatabaseConnection()
	{
		...
		try
		{
			...
			m_dbConnection = CreateConnection(ResolveConnectionType(), text);
