...
public class SolrStatusResponseParser : ISolrStatusResponseParser
{
	...
	{
		XElement statusNode = xml.XPathSelectElement("response/lst[@name='status']");
		if (statusNode == null || !statusNode.HasElements)
		{
			return new List<CoreResult>();
		}
		return statusNode.Elements().Select(ParseCore).ToList();
	}
	...
	private static CoreResult ParseCore(XElement node)
	{
		...
		foreach (XElement propNode in node.Elements())
		{
			string nodeValue = propNode.Value;
	...
	{
		CoreIndexResult coreIndex = new CoreIndexResult();
		foreach (XElement indexNode in node.Elements())
		{
			string nodeValue = indexNode.Value;
