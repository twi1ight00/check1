...
public static class ExtensionElementMap
{
	private class ExtensionElementMapImpl
	{
		...
		public string GetTagForExtensionElement(ConfigurationElement element)
		{
			...
			Dictionary<string, Type> dictToSearch = GetDictToSearch(type);
			foreach (KeyValuePair<string, Type> item in dictToSearch)
			{
				if (item.Value == type)
				{
					return item.Key;
			...
			throw ElementTypeNotFound(type);
		}
		...
		{
			return new ArgumentException(string.Format(CultureInfo.CurrentCulture, Resources.ElementTypeNotRegistered, elementType), "memberElement");
		}
