...
public class SolrDIHStatusParser : ISolrDIHStatusParser
{
	...
	{
		SolrDIHStatus result = new SolrDIHStatus();
		if (solrDIHStatusXml == null)
		{
			return result;
		}
		foreach (XElement fieldNode in solrDIHStatusXml.Element("response").Elements("str"))
		{
		...
		foreach (XElement fieldNode in solrDIHStatusXml.XPathSelectElement("//lst[@name='statusMessages']").Elements())
		{
			DateTime tempDate;
			switch (fieldNode.Attribute("name").Value)
			{
			case "Time Elapsed":
			{
				string[] tempTimeSpanSplit = fieldNode.Value.Split(':');
