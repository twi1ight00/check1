...
public class XmlHierarchyConfigurator
{
		...
		try
		{
			...
			foreach (XmlNode childNode in appenderElement.ChildNodes)
			{
				...
				if (xmlElement.LocalName == "appender-ref")
				{
					...
					if (appender is IAppenderAttachable appenderAttachable)
					{
	...
	protected void SetParameter(XmlElement element, object target)
	{
		...
		if (text2 != null)
		{
			...
			if (obj != null)
			{
				...
				if (methodInfo != null)
				{
					...
					{
						LogLog.Error(declaringType, string.Concat("Failed to set parameter [", text, "] on object [", target, "] using value [", obj, "]"), ex2.InnerException);
						return;
					}
				}
			}
		...
		if (propertyInfo != null)
		{
			...
			{
				LogLog.Error(declaringType, string.Concat("Failed to set parameter [", propertyInfo.Name, "] on object [", target, "] using value [", obj2, "]"), ex2.InnerException);
				return;
			}
		}
		if (!(methodInfo != null))
		...
		{
			LogLog.Error(declaringType, string.Concat("Failed to set parameter [", methodInfo.Name, "] on object [", target, "] using value [", obj2, "]"), ex2.InnerException);
		}
	}
