...
internal class JsonSchemaWriter
{
	...
	{
		IList<JsonSchemaType> list = ((!Enum.IsDefined(typeof(JsonSchemaType), type)) ? (from v in EnumUtils.GetFlagsValues(type)
			where v != JsonSchemaType.None
			select v).ToList() : new List<JsonSchemaType> { type });
		if (list.Count == 0)
		{
			return;
		}
		...
		foreach (JsonSchemaType item in list)
		{
