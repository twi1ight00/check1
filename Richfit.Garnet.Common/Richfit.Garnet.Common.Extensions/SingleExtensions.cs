...
/// <summary>
/// <see cref="T:System.Single" /> 类型扩展方法类
/// </summary>
public static class SingleExtensions
{
	...
	public static byte[] GetBytes(this float[] values)
	{
	...
	public static byte[] GetBytes(this float[] values, int start, int count)
	{
		values.GuardNotNull("values");
	...
	{
		values.GuardNotNull("values");
		return values.SelectMany((float x) => x.GetBytes()).ToArray();
	}
	...
	public static byte[] GetRawBytes(this float value)
	{
	...
	/// <returns>Big-Endian 顺序的字节数组</returns>
	/// <exception cref="T:System.ArgumentNullException">当前数组为空。</exception>
	public static byte[] GetRawBytes(this float[] values)
	{
		values.GuardNotNull("values");
	...
	/// <exception cref="T:System.IndexOutOfRangeException">开始转换的索引 <paramref name="start" /> 超出了当前数组的索引范围。</exception>
	/// <exception cref="T:System.ArgumentOutOfRangeException">转换的值数量 <paramref name="count" /> 小于0，或者大于从 <paramref name="start" /> 起剩余的值数量。</exception>
	public static byte[] GetRawBytes(this float[] values, int start, int count)
	{
		values.GuardNotNull("values");
		start.GuardIndexRange(0, values.Length - 1, "start");
		count.GuardBetween(0, values.Length - start, "count");
	...
	/// <returns>Big-Endian 顺序的字节数组</returns>
	/// <exception cref="T:System.ArgumentNullException">当前序列为空。</exception>
	public static byte[] GetRawBytes(this IEnumerable<float> values)
	{
		values.GuardNotNull("values");
		return values.SelectMany((float x) => x.GetRawBytes()).ToArray();
	}
	...
	public static string ToHex(this float value, bool upperCase = false)
	{
	...
	/// <param name="upperCase">是否转换为大写的十六进制字符串形式</param>
	/// <returns>当前数值的十六进制字符串形式</returns>
	public static string ToHex(this float value, string prefix, bool upperCase = false)
	{
