...
internal class Class1890 : Class1351
{
	public delegate Class1890 Delegate1537();

	public delegate void Delegate1539(Class1890 elementData);

	public delegate void Delegate1538(Class1890 elementData);
		...
		while (reader.MoveToNextAttribute())
		{
			if (!(reader.Prefix == "xmlns"))
			{
				...
				case "sy":
					...
					break;
				case "kx":
					float_8 = (float)XmlConvert.ToInt32(reader.Value) / 60000f;
					break;
				case "ky":
					float_9 = (float)XmlConvert.ToInt32(reader.Value) / 60000f;
					break;
				case "algn":
	...
	public Class1890(XmlReader reader)
		: base(reader)
	{
	}
	...
	public override void vmethod_4(string prefix, XmlWriter writer, string elementName)
	{
		...
		if (float_5 != 0f)
		{
			writer.WriteAttributeString("dir", XmlConvert.ToString((int)Math.Round(float_5 * 60000f)));
		}
		if (float_6 != 100f)
		{
			writer.WriteAttributeString("sx", XmlConvert.ToString((int)Math.Round(float_6 * 1000f)));
		}
		if (float_7 != 100f)
		{
			writer.WriteAttributeString("sy", XmlConvert.ToString((int)Math.Round(float_7 * 1000f)));
		}
		if (float_8 != 0f)
		{
			writer.WriteAttributeString("kx", XmlConvert.ToString((int)Math.Round(float_8 * 60000f)));
		}
		if (float_9 != 0f)
		{
			writer.WriteAttributeString("ky", XmlConvert.ToString((int)Math.Round(float_9 * 60000f)));
		}
