...
{
	private readonly Dictionary<HandlerPipelineKey, HandlerPipeline> pipelines = new Dictionary<HandlerPipelineKey, HandlerPipeline>();
	...
	public HandlerPipeline GetPipeline(MethodBase method)
	{
		...
		HandlerPipeline result = emptyPipeline;
	...
	{
		HandlerPipeline handlerPipeline = CreatePipeline(method.ImplementationMethodInfo, handlers);
		if (method.InterfaceMethodInfo != null)
		{
			pipelines[HandlerPipelineKey.ForMethod(method.InterfaceMethodInfo)] = handlerPipeline;
		}
		return handlerPipeline.Count > 0;
	}
	...
	private HandlerPipeline CreatePipeline(MethodInfo method, IEnumerable<ICallHandler> handlers)
	{
		...
		HandlerPipeline handlerPipeline = CreatePipeline(method.GetBaseDefinition(), handlers);
		pipelines[key] = handlerPipeline;
