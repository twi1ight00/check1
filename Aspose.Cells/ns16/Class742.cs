...
internal static class Class742
{
	...
	internal static byte[] smethod_2(string string_0, Encoding encoding_2)
	{
	...
	{
		return smethod_2(string_0, encoding_0);
	}

	internal static string smethod_4(byte[] byte_0)
	{
		return smethod_5(byte_0, encoding_1);
	}

	internal static string smethod_5(byte[] byte_0, Encoding encoding_2)
	{
		return encoding_2.GetString(byte_0, 0, byte_0.Length);
	}

	internal static int smethod_6(Stream stream_0)
	{
		int result = 0;
		try
		{
			result = smethod_9(stream_0, "n/a");
		}
		catch (Exception3)
		{
		}
	...
	internal static int smethod_7(Stream stream_0)
	{
		int num = 0;
		try
		{
			num = smethod_9(stream_0, "n/a");
			if (num == 134695760)
			{
	...
	{
		return smethod_9(stream_0, "Could not read block - no data!  (position 0x{0:X8})");
	}

	private static int smethod_9(Stream stream_0, string string_0)
	{
		int num = 0;
		...
		if (num != array.Length)
		{
			throw new Exception3(string.Format(string_0, stream_0.Position));
		}
		return ((array[3] * 256 + array[2]) * 256 + array[1]) * 256 + array[0];
	}

	internal static long smethod_10(Stream stream_0, int int_0)
	{
		...
		return stream_0.Position - position - 4;
	}

	internal static DateTime smethod_11(DateTime dateTime_0)
	{
	...
	internal static DateTime smethod_12(int int_0)
	{
		if (int_0 != 65535 && int_0 != 0)
		{
			short num = (short)(int_0 & 0xFFFF);
			short num2 = (short)((int_0 & 0xFFFF0000u) >> 16);
			...
			if (!flag)
			{
				...
				throw new Exception1($"Bad date/time format in the zip file. ({arg})");
			}
	...
	internal static int smethod_13(DateTime dateTime_0)
	{
	...
	public static void smethod_14(string dir, out Stream fs, out string filename)
	{
		for (int i = 0; i < 3; i++)
		{
			try
			{
				filename = Path.Combine(dir, smethod_15());
	...
	internal static int smethod_16(Stream stream_0, byte[] byte_0, int int_0, int int_1, string string_0)
	{
		int result = 0;
		bool flag = false;
		int num = 0;
	...
	private static uint smethod_17(Exception exception_0)
	{
